# coding: utf-8

import os
import re
import argparse
import pdb
import time
import numpy as np
import logging
import mxnet as mx
from mxnet import nd, gluon, autograd, image
from mxnet.gluon import nn, data as gdata
import matplotlib.pyplot as plt
from tqdm import tqdm
from util.misc import CSVLogger
from Data import *
from debug_code import *

args = argparse.ArgumentParser()
args.add_argument("--ckpt",default=None,
                  help="path to checkpoint")
args.add_argument("--log",default=None,
                  help="path to log")
args.add_argument("--use_cutout", action='store_true', default=False,
                  help="apply cutout or not")
args.add_argument("-e", "--epochs", type=int, default=200,
                  help="num_eopchs")
args.add_argument("--gpu", type=int, default=0,
                  help="gpu id")
args.add_argument("-o","--optimizer", default="sgd",
                  help="optimizer used in training.")
args.add_argument("-s", "--model_saved", default='./models_cutout',
                  help="path to model_saved")
args.add_argument("--test_only", action='store_true', default=False,   # 注意应该是store true
                  help='set script to testing mode')
args = args.parse_args()



def get_transforms(use_cutout=False):
    trans_train = gdata.vision.transforms.Compose([   # 注意Compose接收的是一个list
        Pad(pad_width=(4,4,0)),
        gdata.vision.transforms.RandomResizedCrop(32, scale=(0.64, 1.0),ratio=(1.0,1.0)),
        gdata.vision.transforms.RandomFlipLeftRight(),
        gdata.vision.transforms.ToTensor(),
        gdata.vision.transforms.Normalize([0.4914, 0.4822, 0.4465],
                               [0.2023, 0.1994, 0.2010]),
    ])
    if use_cutout:
        trans_train.add(Cutout(n_holes=1, length=16))

    trans_test = gdata.vision.transforms.Compose([
        gdata.vision.transforms.ToTensor(),
        gdata.vision.transforms.Normalize([0.4914, 0.4822, 0.4465],
                                          [0.2023, 0.1994, 0.2010]),
    ])
    return trans_train, trans_test


def show_images(imgs, num_rows, num_cols, scale=2):
    """Plot a list of images."""
    figsize = (num_cols * scale, num_rows * scale)
    _, axes = plt.subplots(num_rows, num_cols, figsize=figsize)
    for i in range(num_rows):
        for j in range(num_cols):
            axes[i][j].imshow(imgs[i * num_cols + j].asnumpy())
            axes[i][j].axes.get_xaxis().set_visible(False)
            axes[i][j].axes.get_yaxis().set_visible(False)
    return axes

class Residual(nn.HybridBlock):
    def __init__(self, num_channels, use_1x1conv=False, strides=1, **kwargs):
        super(Residual, self).__init__(**kwargs)
        self.conv1 = nn.Conv2D(num_channels, kernel_size=3, padding=1,
                               strides=strides)
        self.conv2 = nn.Conv2D(num_channels, kernel_size=3, padding=1)
        if use_1x1conv:
            self.shortcut = nn.HybridSequential()
            self.shortcut.add(
                nn.Conv2D(num_channels, kernel_size=1, strides=strides),
                nn.BatchNorm()
            )
        else:
            self.shortcut = None
        self.bn1 = nn.BatchNorm()
        self.bn2 = nn.BatchNorm()
        
    def hybrid_forward(self, F, X):
        Y = F.relu(self.bn1(self.conv1(X)))
        Y = self.bn2(self.conv2(Y))
        if self.shortcut:
            X = self.shortcut(X)
        return F.relu(Y + X)
def resnet18(num_classes):
    net = nn.HybridSequential()
    net.add(nn.Conv2D(64, kernel_size=3, strides=1, padding=1),
            nn.BatchNorm()#, nn.Activation('relu')
            )
    def resnet_block(num_channels, num_residuals, first_block=False):
        blk = nn.HybridSequential()
        for i in range(num_residuals):
            if i == 0 and not first_block:
                blk.add(Residual(num_channels, use_1x1conv=True, strides=2))
            else:
                blk.add(Residual(num_channels))
        return blk
    
    net.add(resnet_block(64, 2, first_block=True),
            resnet_block(128, 2),
            resnet_block(256, 2),
            resnet_block(512, 2))
    #net.add(nn.Dense(1024))
    net.add(nn.GlobalAvgPool2D())
    #net.add(nn.Activation('relu'))
    net.add(nn.Dense(num_classes))
    return net

def get_net(ctx, lr, ckpt=None):
    num_classes = 10
    net = resnet18(num_classes)
    if ckpt is not None:
        print('[+]loading ckpt from {}'.format(ckpt))
        net.load_parameters(ckpt, ctx=ctx)
        #pdb.set_trace()
        start_epoch = int(re.findall(r'epoch\_(\d+)', ckpt)[0])
        print("Load checkpoint from {}".format(ckpt))
    else:
        net.initialize(ctx=ctx, init=mx.init.Xavier())
        start_epoch = 1
    print('[+]Start training from epoch {}'.format(start_epoch))
    
    points = list(lr_decay_dict.keys())
    index = [start_epoch>l for l in points]   # 注意这里设置 > 而不是 >=
    #即当start_epoch=60时，这里设置学习率为0.1(未衰减),因为到train loop里面会遇到decay point再衰减
    keys=np.array(points)[index].tolist()
    for k in keys:
        lr = lr * lr_decay_dict[k]
    print('[+]Starting learning rate : {}'.format(lr))
    return net, start_epoch, lr
        


def evaluate_accuracy(data_iter, net, ctx=[mx.cpu()]):
    """Evaluate accuracy of a model on the given data set."""
    acc = nd.array([0], ctx=ctx)
    n = 0
    for X, y in data_iter:
        X = X.as_in_context(ctx)
        y = y.astype('float32').as_in_context(ctx)
        acc += (net(X).argmax(axis=1) == y).sum()
        n += y.size
    return acc.asscalar() / n

def plot_history(train_loss_history, train_acc_history, val_acc_history):
    #plt.clf()
    ax_epoch = list(range(len(train_loss_history)))
    #pdb.set_trace()
    plt.subplot(1, 2, 1);
    plt.plot(ax_epoch, train_acc_history, 'r-');
    #plt.hold()
    plt.plot(ax_epoch, val_acc_history, 'g-.');
    plt.legend(labels=['train_acc','val_acc'], loc='upper right')
    plt.subplot(1, 2, 2);
    plt.plot(ax_epoch, train_loss_history, 'b-');
    plt.legend(labels = ['train loss'], loc = 'upper right')
    plt.show();
    plt.pause(0.00001)

def train(start_epoch, lr, lr_decay_dict, ctx, num_epochs, train_iter, val_iter, trainer, metric, logger, save_name=None):
    if not os.path.exists(os.path.dirname(save_name)):
        os.makedirs(os.path.dirname(save_name))
    train_loss_history = []
    train_acc_history = []
    val_acc_history = []
    for epoch in range(start_epoch, num_epochs+1):
        train_loss_avg = 0
        progress_bar = tqdm(train_iter)
        tic = time.time()
        metric.reset()
        info_list = ''
        for iter, (X, y) in enumerate(progress_bar):
            progress_bar.set_description('Epoch ' + str(epoch))
            X = X.as_in_context(ctx)
            y = y.as_in_context(ctx)
            #y = nd.one_hot(y, 10).as_in_context(ctx)
            with autograd.record():
                logits = net(X)
                l = loss(logits, y)
                l.backward()
            train_loss_avg += l.mean()
            trainer.step(batch_size)
            metric.update([y,],[logits,])
            name, train_acc = metric.get()
            progress_bar.set_postfix(
                train_loss='%.3f' % (train_loss_avg.asnumpy() / (iter + 1)),
                acc='%.3f' % train_acc)
            # if iter % 100 == 0:
            #     name,train_acc = metric.get()
            #
            #     #info_iter = 'Epoch {}, Iter {}, train acc {}, train loss {}'.format(epoch, iter, train_acc, l.mean().asscalar())
            #     print(info_iter)
            #     info_list += info_iter+'\n'
        #下面的写错了
        #if epoch in lr_decay_dict.keys():
        #    for point in sorted(lr_decay_dict.keys()):
        #        if epoch > point:
        #            trainer.set_learning_rate(trainer.learning_rate * lr_decay_dict[point])
        #    trainer.set_learning_rate(trainer.learning_rate * lr_decay_dict[epoch])  # lr schedule
        #下面的可以实现，不过不够简洁
        '''
        decay=1
        if epoch in lr_decay_dict.keys():
            for point in sorted(lr_decay_dict.keys()):
                if epoch >= point: 
                    decay *=  lr_decay_dict[point]
        trainer.set_learning_rate(lr * decay)
        '''
        if epoch in lr_decay_dict.keys():
            trainer.set_learning_rate(trainer.learning_rate * lr_decay_dict[epoch])

        print('[+]Changing learning rate to {}'.format(trainer.learning_rate))
        if ((epoch % 50 == 0) and (epoch != start_epoch)): # or (epoch % 5 ==0 and epoch < 20)
            #trainer.set_learning_rate(trainer.learning_rate * lr_decay)
            net.save_parameters(save_name+'_epoch_{}'.format(epoch))
        valid_acc = evaluate_accuracy(val_iter, net, ctx)
        tqdm.write('val_acc: %.3f' % (valid_acc))
        csv_row = {'epoch':str(epoch), 'train_acc':str(train_acc), 'valid_acc':str(valid_acc), 'lr':str(trainer.learning_rate)}
        logger.writerow(csv_row)
        #train_loss_history.append(l.mean().asscalar())
        #train_acc_history.append(train_acc)
        #val_acc_history.append(valid_acc)
        #plot_history(train_loss_history, train_acc_history, val_acc_history)
    #plt.savefig('logs/train_cifar10_res18_scheDict.png')
            
if __name__=='__main__':
    batch_size = 128
    ctx = mx.gpu(args.gpu)
    lr = 0.1
    wd = 5e-4  # 从5e-4 改成1e-4
    lr_decay_dict = {60:0.2, 120:0.2, 160:0.2}
    Train_Root = '../../Dataset/Train/'
    Val_Root = '../../Dataset/Valid/'
    #Train_Val_Root = '../../Dataset/Total/'
    #plt.ion()
    trans_train, trans_test = get_transforms(args.use_cutout)
    train_iter, val_iter = load_dataset(Train_Root, Val_Root, batch_size=batch_size, trans_train=trans_train, trans_test=trans_test)
    net, start_epoch, lr = get_net(ctx, lr, args.ckpt)
    
    net.hybridize()  #别忘记加 hybridize()

    trainer = gluon.Trainer(
       net.collect_params(), args.optimizer, #sgd
       {'learning_rate' : lr, 'wd' : wd})
    # schedule = mx.lr_scheduler.MultiFactorScheduler(step=[75,150,450], factor=lr_decay)
    # schedule.base_lr = lr
    # adam_optimizer = mx.optimizer.Adam(wd=wd) #learning_rate=lr, lr_scheduler=schedule,
    # trainer = mx.gluon.Trainer(params=net.collect_params(), optimizer=adam_optimizer)
    #这里trainer先不设置学习率

    logging.basicConfig(level=logging.DEBUG)
    csv_logger = CSVLogger(args=args, fieldnames=['epoch', 'train_acc', 'valid_acc','lr'], filename=args.log)
    metric = mx.metric.Accuracy()
    loss = gluon.loss.SoftmaxCrossEntropyLoss()
    
    logging.info('[+][+]Hyperparameters: lr: {}, wd: {}, lr_decay_dict: {}'.format(lr, wd, lr_decay_dict))
    logging.info('[+]Training Start...')
    if not args.test_only:
        train(start_epoch, lr, lr_decay_dict, ctx, args.epochs, train_iter, val_iter, trainer, metric,
        save_name=args.model_saved, logger=csv_logger)
    else:
        logging.info("Start evaluating ... ")
        valid_acc = evaluate_accuracy(val_iter, net, ctx)
        logging.info("Final model valid accuracy: {:.5f}".format(valid_acc))
